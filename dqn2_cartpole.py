...
class DQN:
    ...
    def select_action(self, state, action_space):
        with torch.no_grad():
            ...
            policy = F.softmax(Q_value / self.alpha)
    ...
    def calculate_q_value(self, Q_value, policy, action):
        entropy = -torch.sum(policy * torch.log(policy + 1e-34), 1).reshape(self.batch_size, 1)
        value = torch.sum(policy * Q_value, 1).reshape(self.batch_size, 1)
        pi = policy.gather(1, action.long())
        return self.alpha * (torch.log(pi + 1e-34) + entropy) + value
    ...
    def _update_behavior_network(self):
        ...
        with torch.no_grad():
            ...
            policy_next = F.softmax(Q_next / self.alpha)
            q_next = self.calculate_q_value(Q_next, policy_next, action)
...
def main():
    ...
    parser.add_argument('--alpha', default=.05, type=float)
