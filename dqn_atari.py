...
class Net(nn.Module):
    ...
    def forward(self, x):
        x = x / 255
        out = self.cnn(x)
        out = out.reshape(out.size(0), 128 * 9 * 6)
        return self.fc(out)
...
class DQN:
    ...
    def _update_behavior_network(self, gamma):
        ...
        state = torch.moveaxis(state, 3, 1) # batch * 64 * 64 * 3 -> batch * 3 * 64 * 64
...
def main():
    ...
    parser.add_argument('--epsilon', default=.01, type=float)
